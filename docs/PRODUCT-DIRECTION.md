...
Provider account management and unified account-limit history remain unfinished. The native collectors attach owner-local quota readings after merging supported peer records. Quota history does not enter the current peer payload, so pairing alone does not synchronize account limits. The legacy Mac collector saved real retained allowance observations on September 12, including a background reading. Native graphs have passed synthetic visual checks. These checks do not establish a clean public release. Existing collectors and stored records must be preserved while those paths are replaced.
