...
The legacy Mac collector continues recording real allowance observations and configured cross-device sources. It also supports agent receipts and local-model results that the newer collector does not yet import. Migration must preserve those records, explain any coverage change and retain a recoverable configuration. Do not silently replace this working path just to activate new pairing controls. Verified development packages and installed updates do not establish a clean public release.
