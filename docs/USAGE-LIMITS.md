...
Windows synthetic callback tests cover cancelled consent, enable and disable. The Mac native application compiles and passes self-tests. Its actual Swift-to-Node bridge passes an isolated empty-runtime test for status, disable and rejecting unpaired enable. That test caught and fixed filesystem-alias entry-point handling. Successful paired enable, Mac visual interaction and the Windows packaged bridge remain unverified.
