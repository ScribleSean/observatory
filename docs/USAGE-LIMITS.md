...
Windows synthetic callback tests cover cancelled consent, enable and disable. Both native-to-Node bridges now pass isolated empty-runtime checks against the candidate's actual bundled runtime and scripts for status, disable and rejecting unpaired enable. The Mac bridge test caught and fixed filesystem-alias entry-point handling. Successful paired enable through native controls and Mac sharing-panel visual interaction remain unverified.

Clean source revision `3c81dcb` produced a Windows candidate with 272 passing script tests and 14 platform skips. Its packaged native dashboard, usage popup, setup wizard and sharing bridge passed. A matching Mac ZIP passed manifest, signature, isolated collector, renderer, lifecycle and relocation checks. These are development candidates, not installed updates or public releases. End-to-end consent and allowance exchange over the actual device connection remain release gates.
...
With the corrected Mac source and the Windows candidate endpoint, actual SSH exchange passed bidirectional retention, duplicate delivery and both disable checks. Real accounts were not enabled. The `3c81dcb` candidates predate this fix and must be rebuilt before installation. Successful paired consent through both native interfaces remains a separate verification gate.
...
Both native allowance views now render the shared history beneath local observations, with an explicit source-device label, original reading time and receipt time. They reuse the existing gap/reset-aware graphs without adding device totals. Windows synthetic UI tests cover missing local quota, stale peer history and removing the peer graph. That rendered Windows fixture was visually checked. Mac source compiles and passes native self-tests, but its shared-history visual interaction and real installed-device exchanges remain unverified. Installed apps have not received this code and still do not exchange allowances.
