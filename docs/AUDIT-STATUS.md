...
- Agents now has a separate view in the web source, matching the native navigation. Sources retains connection health and coverage. The existing receipt, benchmark and tool components were reused. Browser checks with fictional records verified both views at desktop and narrow widths, and the narrow receipt metric layout was corrected. The [hosted demo workflow](https://github.com/ScribleSean/observatory/actions/runs/35285460695) passed at `0e11135`, including the complete test suite and TypeScript check. Live browser checks confirmed direct Agents links and Sources navigation. The installed web fallback still requires the updated bundle.
- The explicit Mac web-fallback route still redirected Agents to Sources and sent Allowances and Settings to Activity after the web navigation repair. It now validates against the same seven-section list as the native dashboard, with Allowances as the fallback. Full Swift compilation and native self-tests pass. This source correction is newer than the retained build-26 packages and still needs packaged interaction verification.
