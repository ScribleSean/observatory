...
- Current source still places the development footer on every native Mac section, hardcodes `includeUbuntu: false` in Mac direct-pairing join, and silently exits a foreground Mac launch blocked by the installation gate. These lower-priority findings have not been established as resolved.
