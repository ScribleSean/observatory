...
Launch a development build with `--native-dashboard` to use native Windows controls for Activity, Tokens, Allowances, Dictation and Sources. Quit any running Observatory instance first. The single-instance activation mechanism opens the existing instance and does not change its display mode.
...
This preview is not the installed default. Agent detail and integrated Settings are not migrated yet. Full accessibility review and feature parity remain unfinished. Empty history is explained separately from recorded zero usage, and an unknown quota observation time is identified. It does not add device synchronization or provider sign-ins.
