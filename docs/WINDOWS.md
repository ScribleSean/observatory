...
This preview is not the installed default. Integrated Settings are not migrated yet. Full accessibility review and feature parity remain unfinished. Empty history is explained separately from recorded zero usage, and an unknown quota observation time is identified. It does not add device synchronization or provider sign-ins.
