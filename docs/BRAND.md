...
Use an earthy space palette. Dark mode uses background `#1c1d1b`, raised surfaces `#272825`, text `#f0eee8`, secondary text `#b3b7ac`, sage `#adc29d` and muted purple `#b7a4cc`. Light mode uses background `#edeae5`, surfaces `#f4f1ec`, text `#292d28`, secondary text `#64695f`, sage `#586f50` and purple `#79658e`. The default is dark. Both modes must remain readable.

Keep the mark monochrome. Leave clear space around it and keep the telescope recognizable at small sizes. Do not add galaxies, glows or decorative animation. Bundle Inter Tight locally with its SIL Open Font License. Titles use 22px type, 28px line height, semibold weight and -0.7px tracking. Section headings use 19/24px semibold with 0.6px tracking. Body labels use 14.5/18px regular with 0.3px tracking. These explicit heading tokens take precedence over generic tracking guidance.
...
Navigation order is Allowances, Activity, Tokens, Dictation, Agents, Source health, Settings. Agents contains saved receipts, benchmarks and recorded tool calls. Source health contains connection status, collection health and coverage. Make remaining allowance, estimated time coverage to reset and observed hourly pace easy to scan. Never connect chart segments across missing readings or resets.
