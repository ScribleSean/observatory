...
Status as of September 26, 2026 UTC: Mac runs **0.3.13, build 35** from `ced669854940f5892570a6a7a8feb1689b903b80`, with scheduled local Claude and Antigravity checks passed. Windows retains the verified **build 34** payload from `80a7aa8397a208fad51599f990b094af3d29458f`, with its app stopped at the build-35 uninstall guard. Claude sharing is off on both devices. Neither is published as a production release.
...
The [Mac build-35 run](https://github.com/ScribleSean/observatory/actions/runs/36217710780) passed full packaging, updater smoke checks and distributable verification at `ced6698`. At 06:42:16 UTC, the Mac owner verified the installed build-35 payload before launch, including all 1,874 files, 197,560,395 unpacked bytes and strict signatures. Build 34 was retained and verified for recovery. Current saved state matched its protected backup under collection, peer-state and installer guards, which released normally. Configuration was unchanged.

The owner confirmed a normal launch of the exact installed process. Ordinary scheduled collection from 07:36:46 to 07:37:09 UTC read local Claude and Antigravity successfully. Schema, private arithmetic and Codex-separation checks passed. Health was 11 of 15 sources, with three Ubuntu sources and the agent source unavailable. Claude and Antigravity remained enabled, with sharing off. Normal Quit succeeded, but subsequent launch capture failed after the correct process started. New build-35 UI checks remain unobserved. That capture failure does not establish an application defect.

The [Windows build-35 run](https://github.com/ScribleSean/observatory/actions/runs/36217712159) at the same source failed its full-package step at 04:47 UTC. The main suite passed 581 of 634 tests, with 53 skips and no failures. The isolated `peer-pairing` file then reached an outer 30-second timeout, reporting two passed, one skipped and one cancelled test. No Windows candidate, installer or lifecycle evidence was produced. The unchanged named test passed locally on Windows in 15.165 seconds. A [bounded hosted diagnostic](https://github.com/ScribleSean/observatory/actions/runs/36221939815) then established that the cumulative 30-second fixture budget was insufficient. [PR #17](https://github.com/ScribleSean/observatory/pull/17) split the fixture into five 30-second phases with cancellation-aware cleanup, preserving the 20-second child and 15-second ACL timeouts. The [focused hosted check](https://github.com/ScribleSean/observatory/actions/runs/36222438510) passed all six tests. The specific final ACL error cause remains unproven.

The [successful Windows build-35 run](https://github.com/ScribleSean/observatory/actions/runs/36222564974), attempt 1, passed full packaging, the TEST installer lifecycle and both artifact-retention uploads at `7c7ae25541b8b6cc3c2529b6de5fc67de0f656a8`. The release coordinator independently verified ZIP digests, the complete 3,079-file inventory covering 268,990,091 unpacked bytes, and the ordinary installer and build receipt. The Windows owner independently confirmed candidate version `0.3.13.35`.

The local Windows activation then stopped at the uninstall guard. Read-only checks confirmed the complete installed build-34 payload and matching protected backup remained intact. The ordinary app is stopped, and a temporary NSIS uninstaller child remains alive. The activation helper quoted `/S`, preventing recognition of the silent switch, and observed the self-copy parent instead of the real child. This is a local activation-helper defect, separate from the successful hosted package. The exact dialog page is unobserved because supported Windows computer control is unavailable.

One normal manual Cancel or close has been requested. No forced stop, retry, install or relaunch is authorized while that child remains. After safe cancellation and revalidation, the corrected continuation will use the maintained external copied-uninstaller route with `/S _?=` and observe the real exit. Windows build 35 is not installed, and its scheduled Claude acceptance has not run. Installed peer sharing and combined Claude totals remain unverified.
...
### Windows build 34 package and guarded installation
...
The [corrected hosted package run](https://github.com/ScribleSean/observatory/actions/runs/36213635154), attempt 1, passed at `80a7aa8397a208fad51599f990b094af3d29458f`. Full packaging ran from 03:04:06 to 03:31:57 UTC. TEST installer and ordinary staging lifecycle checks ran from 03:31:57 to 03:36:04 UTC. Both artifact uploads succeeded, and the job completed at 03:36:25 UTC. This verifies the retained uploads introduced by [PR #5](https://github.com/ScribleSean/observatory/pull/5), merged as `978a0a03611ee2d4d77ef02b1ce0e0f6bd3fa6e5`.

The main JavaScript gate reported 627 tests: 574 passed, 53 skipped and none failed. Three isolated test invocations reported 3 passed and 1 skipped, 18 passed and 1 skipped, and 5 passed with no skips. The additional installation verifier passed all 29 checks.

| Artifact | Size or coverage | SHA-256 |
| --- | --- | --- |
| Windows candidate manifest | 3,079 files, 268,989,246 unpacked bytes | `0f062da6747c3ec6270516dda5b656b70081c45c273b03ead10dbccb6a452cfd` |
| Ordinary Windows installer | 100,461,616 bytes | `22ec17127efb9d7ae80323b74f66aa2dd69f63e8980379e993cc8fe0fa8ab148` |

The release coordinator independently verified the artifact ZIP hashes, strict file manifest and ordinary staging receipt. The verification receipt has SHA-256 `f4d8583f0b41df4f79d10e90327deee430a3eebf3628a6113ca3b92f56271311`. This verifies the candidate package. Hosted lifecycle success remains separate from development-machine installation evidence.

The Windows host owner reported ordinary uninstall and install exit codes of zero and passing exact installed manifest and file-version checks. The first guarded attempt stopped during normal process teardown, before backup or uninstall; continuation verified the original build 31 was unchanged and preserved a protected recovery copy. A later saved-state guard stopped before relaunch because private peer state contained a newer valid observation. Read-only checks found intact databases with matching schema, local data and bindings; all other saved files, configuration, trust and consent matched the backup. The writer is unproven. The coordinator accepted retaining the newer observation alongside the immutable original backup and both failure receipts.

The owner's separate locked continuation passed final installation checks and acknowledged ordinary app readiness at 03:54:11 UTC. It reverified source `80a7aa8397a208fad51599f990b094af3d29458f`, file version `0.3.13.34` and the manifest and installer pins above. Saved data matched a separately protected reconciled baseline, preserving the accepted newer peer observation. Startup settings, approval, shortcuts and ownership were preserved. The continuation used the collection and maintained peer-state locks, without data overwrite, another installer or a forced stop. The Mac's bounded collection pause was released normally, with configuration and saved snapshots unchanged.

Claude remained disabled in the installation receipt. The owner verified inherited launch settings and selected the default local Claude directory because no custom `CLAUDE_CONFIG_DIR` was configured. After enabling local Claude collection, the ordinary scheduled run from 03:59:11.812 to 03:59:26.158 UTC reported Claude unavailable. Overall health was 4 of 8 sources, correctly counting one configured Claude source as unread. Codex and privacy isolation checks passed, and Claude sharing remained off. Read-only metadata and runtime checks ruled out links, an unreadable tree, file-count and total-byte limits, and timezone configuration; parser diagnosis remains pending. Windows installation is accepted, but optional local Claude coverage has not passed. Installed cross-device Claude sharing remains unverified.

The installed Mac keeps its verified `d150587` identity. The test and CI changes in `80a7aa` do not require a Mac rebuild or justify relabeling it. No additional local Windows rebuild is planned. Build 32 and 33 package results remain evidence for their exact revisions and do not establish build 34 acceptance.

### Later source UI verification

[PR #8](https://github.com/ScribleSean/observatory/pull/8), source `c34f012`, merged as `1597978cd4c345a7e3b04419aed27f8ff4de704d` after the build-34 package sources. All four native and updater checks passed. Independent review of fresh fictional Windows captures verified readable Metric and Tokens headings in both themes and the empty-chart sentence, “No saved observations in this period.” Inner scrollbar interaction and offscreen rows remain unverified. See the [source UI evidence](UI-VERIFICATION.md#windows-source-screenshot-review-september-26). These repairs are not in the Mac `d150587` or Windows `80a7aa` build-34 packages.
...
The replacement retained build 29 for recovery and left seven specifically checked configuration, snapshot and private-allowance files unchanged. Scheduled collections at 03:23:18 and 03:28:18 UTC completed partial at 11 of 13 reads. Mac, Windows, and Ubuntu token and settings reads succeeded in the first check. Mac ActivityWatch was unavailable, and the optional agent-receipt reader reported `read-timeout` in both checks. The timeout cause remains unconfirmed. A later app collection at 04:03:08 UTC read 12 of 13 sources, with agent receipts OK and ActivityWatch still unavailable, before a diagnostic shell read or restart.

Mac activity recovered after Rosetta installation and normal launch of the existing Intel ActivityWatch app. The scheduled collection ending at 05:00:18 UTC reported Mac Activity OK, with a matching saved snapshot. Overall status remained 12 of 13 because the Ubuntu local-model benchmark read was unavailable. Native Allowances, Tokens and all seven navigation destinations were observed through accessibility, and the owner confirmed Source health loads. System crash reports established repeated control-helper crashes, so full build-30 UI acceptance remains open.
...
The [hosted Windows verification run](https://github.com/ScribleSean/observatory/actions/runs/35682006145) passed full package, TEST installer lifecycle, and complete synthetic signed-update checks. The ordinary upgrade installed `106f607` as `0.3.13.30`, retained build 29 for recovery, preserved all 306 checked saved-file hashes, and reached normal startup and readiness. Its 03:37:46.819–03:37:51.320 UTC collection read all seven configured sources successfully. The app remains running and the completed launcher was removed. The signed-update lifecycle uses a synthetic signer only; it is not production-update evidence.
