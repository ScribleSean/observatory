...
The [Mac build-35 run](https://github.com/ScribleSean/observatory/actions/runs/36217710780) passed full packaging, updater smoke checks and distributable verification at `ced6698`. The candidate is staged, not installed.

The [Windows build-35 run](https://github.com/ScribleSean/observatory/actions/runs/36217712159) at the same source failed its full-package step at 04:47 UTC. The main suite passed 581 of 634 tests, with 53 skips and no failures. The isolated `peer-pairing` file then reached an outer 30-second timeout, reporting two passed, one skipped and one cancelled test. No Windows candidate, installer or lifecycle evidence was produced. The unchanged named test passed locally on Windows in 15.165 seconds, but the hosted failure cause is not established. The coordinator is preparing a narrow hosted diagnostic.

Both build-35 activations remain held. Package completion, guarded installation and ordinary scheduled Claude acceptance remain separate gates. These results do not establish installed peer sharing or combined Claude totals.
