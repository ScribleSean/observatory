...
The replacement retained build 29 for recovery and left seven specifically checked configuration, snapshot and private-allowance files unchanged. Scheduled collections at 03:23:18 and 03:28:18 UTC completed partial at 11 of 13 reads. Mac, Windows, and Ubuntu token and settings reads succeeded in the first check. Mac ActivityWatch was unavailable, and the optional agent-receipt reader reported `read-timeout` in both checks. The timeout cause remains unconfirmed. A later app collection at 04:03:08 UTC read 12 of 13 sources, with agent receipts OK and ActivityWatch still unavailable, before a diagnostic shell read or restart. The unlocked app exposed its native Allowances view and all seven navigation destinations. Further automated interaction failed at the control connection, so full build-30 UI acceptance remains open.
