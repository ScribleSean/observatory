...
| Saved Claude Code logs | Optional input, cache read, cache write and output token counters | Source health keeps each device separate. Optional paired sharing never adds these records to Codex totals |
...
| Antigravity limits | Current provider-reported remaining percentages and reset times | Optional on Mac through the installed CLI. No token history, account identity, saved history or peer sharing |
...
| Claude Code | Optional saved logs on each collecting device | Optional sharing of per-device records, no cross-device total |
...
| Cursor | Unknown | Individual accounts can offer a [usage CSV export](https://forum.cursor.com/t/usage-not-showing/154766/17), but its exact schema, token arithmetic and repeated-export reconciliation remain unverified here. Team APIs are not assumed |
| Antigravity | Unknown | Current allowance percentages are a separate optional source. They are not token counts |

Antigravity allowance monitoring runs the installed CLI's [non-model usage command](https://www.antigravity.google/docs/cli/commands/usage). The reader requires the inspected structured schema and zero invocation turns and tokens. It bounds runtime and output, verifies consent before discovery and execution, and discards a result if monitoring was disabled during the read. Mac checks run with full collection, normally every five minutes. Native Windows execution remains unsupported until process-tree cleanup is verified. Saved observations keep their original time and become stale after ten minutes. No account history or cross-device combination is inferred from these percentages.
...
The native reader inspects retained session and archive logs on their owning machine. Tokens include all recorded dates. Settings and tool details retain their eight-day window. The existing private cache reuses sanitized events for unchanged files, including on Ubuntu. A cold cache warms within the per-run scan budget and withholds totals until every selected file is covered. No raw conversation text enters the cache. It keeps only model IDs, recorded effort, service tier, token counts and tool-call categories. It never exports prompts, command arguments, working directories or raw session IDs. Duplicate session files select the largest saved copy. Matching turn IDs and timestamped token counters inherited within a recorded fork lineage count once, while unrelated sessions and distinct turns remain independent. Logs without recorded turn IDs cannot establish inherited-event duplication. Turn IDs stay hashed in the private cache and never enter dashboard or peer data. It uses the latest request counters when cumulative usage changes, with cumulative deltas as a fallback. Unchanged cumulative counters are not counted twice. A model selection does not relabel an earlier turn. Both `standard` and `default` are recognized as Standard speed.
