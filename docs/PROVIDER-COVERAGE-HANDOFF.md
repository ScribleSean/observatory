...
Updated September 26, 2026 UTC. [PR #2](https://github.com/ScribleSean/observatory/pull/2) is merged on main at `2292d0d`. Build 33 supersedes build 32 after an accuracy review found that unreadable Claude directories could be silently skipped. This handoff contains source and verification context only.
...
2. The shared desktop build number is now 33. Package both platforms from the same verified revision, then perform the existing recoverable installation procedure. Build 32 package checks remain historical evidence and must not be reused as build 33 verification. Do not modify an installed app bundle in place or enable an unknown configuration key in the build 31 strict Mac validator.
