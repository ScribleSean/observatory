...
- The release ledger distinguishes verified packages, installed applications and provider acceptance. Both devices run build 34. Build 35 source adds the client-error placeholder and sanitized-model reconciliation fixes, with legacy public order preserved. Its packages remain pending.
...
The [build 35 preparation evidence](RELEASE-0.3.13.md#build-35-preparation-september-26-utc) records the matched sanitized Windows blocker, one successful direct candidate read, and source checks. The last installed Windows Claude reading remains unavailable. The earlier [PR #8 screenshot evidence and its limits](UI-VERIFICATION.md#windows-source-screenshot-review-september-26) also remain source-only until a newer package is installed and verified.
...
3. Build and verify exact build-35 packages from the reviewed source, then use guarded upgrades and verify ordinary scheduled Claude collection while preserving saved data. A successful direct candidate read does not establish installed acceptance. The Mac's build-34 Claude and Antigravity checks passed. Mac restart and navigation remain open.
